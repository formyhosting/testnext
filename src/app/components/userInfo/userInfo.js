...
const UserInfo = () => {
  ...
  const [initialDataSize, setInitialDataSize] = useState(0);
  ...
  useEffect(() => {
    ...
    setInitialDataSize(
      performance.getEntriesByType("resource").reduce((acc, entry) => acc + entry.transferSize, 0)
    );
    fetchUserInfo();
  }, [initialDataSize]); // Include initialDataSize in the dependency array
